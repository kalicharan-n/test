...
def layouts():
    ...
    layouts = os.listdir(app.config['UPLOAD_FOLDER'])
    placeholders = list(fake_methods.keys())  # Get list of placeholder keys to pass to the template
    return render_template('layouts.html', layouts=layouts, placeholders=placeholders)
...
def edit_layout(filename):
    ...
    return render_template('edit_layout.html', filename=filename, content=content, placeholders=placeholders)
